...
def get_stock_data():
    ...
    try:
        ...
        symbol = request.args.get('symbol', 'VIX.VN')
...
def predict(symbol, timeframe, data1, data2):
    ...
    model = load_model(f'E:\HOC_MAY\Tuan_13\.ipynb_checkpoints{new_symbol}_{timeframe}.keras')
...
def get_stock_info():
    ...
    try:
        ...
        response = {
            "phamdv_ngay": f"{info.get('dayLow', 0):,.2f} - {info.get('dayHigh', 0):,.2f}",
            "mo_cua": f"{info.get('open', 0):,.2f}",
            "phamvi_52tuan": f"{info.get('fiftyTwoWeekLow', 0):,.2f} - {info.get('fiftyTwoWeekHigh', 0):,.2f}",
            "gia_dong_truoc": f"{info.get('previousClose', 0):,.2f}",
            "kl_trung_binh": f"{info.get('averageVolume', 0) / 1e6:.2f} Tr",
            "von_hoa": f"{info.get('marketCap', 0) / 1e12:.2f} T",
            "co_phieu": f"{info.get('sharesOutstanding', 0) / 1e9:.2f} T",
