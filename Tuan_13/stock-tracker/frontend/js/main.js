...
function loadStockInfo(symbol) {
    fetch(`${API_BASE_URL}/stock-info?symbol=${symbol}`)
        .then(response => response.json())
        .then(data => {
            document.getElementById('stock-info').innerHTML = `
                <div class="stock-stats">
                    <div><strong>Phạm vi ngày:</strong> ${data.phamvi_ngay}</div>
                    <div><strong>Mở:</strong> ${data.mo_cua}</div>
                    <div><strong>Phạm vi 52 tuần:</strong> ${data.phamvi_52tuan}</div>
                    <div><strong>Giá đóng cửa trước đó:</strong> ${data.gia_dong_truoc}</div>
                    <div><strong>Khối lượng trung bình:</strong> ${data.kl_trung_binh}</div>
                    <div><strong>Giá trị vốn hóa thị trường:</strong> ${data.von_hoa}</div>
                    <div><strong>Cổ phiếu đang lưu hành:</strong> ${data.co_phieu}</div>
                    <div><strong>P/E (12 tháng gần nhất):</strong> ${data.pe}</div>
                    <div><strong>EPS (12 tháng gần nhất):</strong> ${data.eps}</div>
                </div>
            `;
        })
        .catch(error => {
            console.error('Lỗi khi lấy thông tin tài chính:', error);
            document.getElementById('stock-info').innerHTML = '<p>Lỗi tải thông tin tài chính</p>';
        });
}
