...
import pandas as pd
import matplotlib
matplotlib.use('Agg')  # Fix lỗi Qt platform plugin
...
model = load_model('/home/levanhuy/Machine_Learning/Tuan_5/my_model.keras')
...
def index():
    if request.method == 'POST':
        ...
        for _ in range(180):  # Predict for 180 days (approx. 6 months)
            last_100_days_reshaped = np.array(last_100_days).reshape(1, 100, 1)
            next_pred = model.predict(last_100_days_reshaped)[0][0]
            future_predictions.append(next_pred)
            last_100_days = np.vstack((last_100_days[1:], [[next_pred]]))
        future_predictions = np.array(future_predictions) * scale_factor
        
         # Plot Future Predictions
        fig4, ax4 = plt.subplots(figsize=(12, 6))
        ax4.plot(range(len(y_test)), y_test, 'g', label="Original Price")
        ax4.plot(range(len(y_test), len(y_test) + len(future_predictions)), future_predictions, 'b', label="Future Predictions")
        ax4.set_title("Stock Price Prediction for Next 6 Months")
        ax4.set_xlabel("Time")
        ax4.set_ylabel("Price")
        ax4.legend()
        future_prediction_chart_path = "static/future_stock_prediction.png"
